...
### ⚠️ Important: Password Handling Policy

**User-provided passwords (root CA passwords, API keys) are NEVER logged or persisted to disk**, except where explicitly required:

- **Root CA password**: Accepted via HTML form, used to derive encryption key via HMAC-SHA256, cleared from memory after use
- **Logging**: If a password appears in logs, it's a bug — report to security team
- **Storage**: Only filesystem-generated passwords are stored in `.pwd.enc` (encrypted at rest)
- **Auditing**: Audit logs record "password provided" or "password incorrect", never the actual password

**Operators**: Configure log aggregation to exclude any sensitive fields. If passwords appear in logs, immediately rotate affected credentials.
...
### 11. Root CA Private Key Access Control (Two-Factor)

**Vulnerability**: Root CA private key (used to issue intermediate CAs and revoke root-issued certificates) is the most critical asset in the PKI. Access should require human authorization beyond filesystem access.

**Design — Two-Factor Protection**:

Root CA private key is protected by BOTH:
1. **Filesystem password** (generated by system, stored in `.pwd.enc`)
2. **User password** (user provides at Root CA creation and each operation)

**Implementation**:

**At Root CA Creation Time:**
```
User provides: root_ca_password (via form, e.g., "MySecurePassword123")
System generates: fs_password (random 32 bytes, hex-encoded)

Effective key = HMAC-SHA256(key=fs_password, msg=root_ca_password)

Encrypt root private key: AES-256-GCM(private_key, effective_key)
Store filesystem password in .pwd.enc (encrypted at rest)

⚠️  User password is NEVER stored, NEVER logged, exists only in-memory
```

**When Using Root CA (Create Intermediate or Revoke):**
```
Read from disk: fs_password from .pwd.enc
User provides: root_ca_password (via form)

Effective key = HMAC-SHA256(key=fs_password, msg=root_ca_password)

Decrypt root private key: AES-256-GCM.decrypt(ciphertext, effective_key)
  └─ Wrong password → invalid effective key → decryption fails → HTTP 500
  └─ Correct password → decryption succeeds → operation proceeds
```

**Affected Routes**:
- `POST /organizations/{org_id}/root-ca` — user sets password for Root CA
- `POST /organizations/{org_id}/intermediate-ca` — user provides password to unlock Root CA private key for signing
- `POST /organizations/{org_id}/certificates/{cert_id}/revoke` — user provides password to unlock Root CA private key for CRL regeneration

**Security Properties**:
- **Dual-factor**: `effective_key` requires BOTH fs_password (disk) AND user_password (user's memory)
- **In-memory only**: User password never persisted to disk or logs
- **No info leakage**: Wrong password → generic HTTP 500 error (no indication of correctness)
- **No bypass**: Private key cannot be decrypted without correct derived key; fails at crypto layer
- **Attestation**: Each use of root key requires explicit user authorization via password entry

**Error Handling**:
- Missing/empty password at creation → HTTP 422
- Missing/empty password at operation → HTTP 422 (for root-issued cert revocation) or error page (intermediate creation)
- Wrong password → HTTP 500 (AES-256-GCM decryption fails, no further details revealed)

**Intermediate CA Private Keys (Not Dual-Factor)**:
Intermediate CAs do NOT use user passwords. Their private keys are encrypted ONLY with their filesystem password:
```
